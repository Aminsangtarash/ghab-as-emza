...
import { adminNav, panelLabel } from "@/lib/account";
...
export function AdminShell({ user, children }: { user: PublicUser; children: React.ReactNode }) {
  ...
  return (
    <SiteViewport>
      <aside
        className={cn(
          "fixed inset-y-2.5 start-2.5 z-40 w-[min(18.5rem,calc(100%-1.25rem))] flex-col rounded-[1.6rem] bg-gold text-navy-deep shadow-xl md:inset-y-3 md:start-3 lg:flex lg:w-72",
      ...
      <div className="flex h-full min-w-0 flex-col overflow-hidden lg:ps-[calc(18rem+0.75rem)]">
        ...
        <GoldCanvas className="px-5 pb-8 pt-8 sm:px-8 sm:pt-10 lg:px-12 lg:pt-12">
    ...
    <>
      <div className="px-5 pt-7">
        <div className="flex items-start justify-between">
          <p className="text-[10px] font-semibold tracking-[0.16em] text-navy/50">قبل از امضا</p>
          <button
            type="button"
        ...
        <div className="mt-6 rounded-xl bg-navy px-4 py-4 text-white">
          <span className="flex size-11 items-center justify-center rounded-lg bg-gold text-navy-deep">
            <ShieldIcon className="size-5" />
          </span>
          <p className="mt-3 text-xs font-medium text-gold">
            {user.role === "manager" ? "مدیر دفتر" : "ادمین سیستم"}
          </p>
          <p className="mt-1 font-heading text-base font-semibold">{user.fullName}</p>
        </div>
      </div>

      <nav className="mt-5 min-h-0 flex-1 px-3" aria-label="منوی مدیریت">
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ...
                      : "text-navy-deep/75 hover:bg-navy/8 hover:text-navy-deep",
                  )}
                >
      ...
      <div className="border-t border-navy/15 p-3">
        <Link
          href="/"
          className="block rounded-lg px-3 py-2.5 text-sm text-navy-deep/70 hover:bg-navy/8 hover:text-navy-deep"
        >
          بازگشت به سایت
        </Link>
        <button
          type="button"
          onClick={onLogout}
          className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-navy-deep/70 hover:bg-navy/8 hover:text-navy-deep"
        >
